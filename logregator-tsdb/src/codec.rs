...
mod test {
    ...
    proptest! {
        ...
        fn prop_framed_writer_reader_roundtrip(
            ...
            let reader = FramedReader::new(Cursor::new(bytes), RecordCodec);
    ...
    fn framed_reader_grows_buffer_even_after_prior_compaction() {
        ...
        let reader: FramedReader<Cursor<Vec<u8>>, RecordCodec, Record> =
            FramedReader::new(Cursor::new(bytes), RecordCodec);
        let mut decoded = vec![];
        for r in reader {
            let r = r.unwrap();
            decoded.push(r);
