...
impl Record {
    ...
    pub fn extract_key(&self) -> anyhow::Result<&[u8]> {
        ...
        let key_len = u32::from_be_bytes([self.0[24], self.0[25], self.0[26], self.0[27]]) as usize;
    ...
    pub fn extract_all_fields_owned(&self) -> anyhow::Result<(i64, i64, u64, String, String)> {
        let (sid, ts, seq, _key, _val) = self
            .extract_all_fields_ref()
    ...
    #[allow(clippy::type_complexity)]
    pub fn extract_all_fields_ref(&self) -> anyhow::Result<(i64, i64, u64, &[u8], &[u8])> {
        if self.0.len() < 24 {
    ...
    #[allow(clippy::type_complexity)]
    pub fn extract_all_fields_no_value_ref(&self) -> anyhow::Result<(i64, i64, u64, &[u8])> {
        if self.0.len() < 24 {
            bail!("record.extract_all: not enough bytes for integer fields")
        }
        let source_id = i64::from_be_bytes([
