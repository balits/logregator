...
pub struct Backend {
    ...
    io_cmd_sender: mpsc::Sender<IoCommand>,
...
impl Backend {
    ...
    pub async fn run_engine_loop(&mut self, mut cmd_recv: mpsc::Receiver<proto::Command>) {
        let mut cmd_buf = Vec::with_capacity(self.config.command_batch_size);

        if !self.engine.is_memtable_empty() {
            self.send_flush_cmd().await.ok();
        }
        ...
        loop {
            ...
            tokio::select! {
                    ...
                    match result {
                       IoResult::Flush { new_meta, start_time } => {
                            if self.apply_flush_result(new_meta, start_time).ok() == Some(true) {
                                self.send_compaction_cmd().await.ok();
                            }
                ...
                n = cmd_recv.recv_many(&mut cmd_buf, self.config.command_batch_size) => {
                    if n == 0 { continue }
            ...
            if needs_flush {
                let _ = self.send_flush_cmd().await;
            }
    ...
    fn handle_range(
        ...
        let _t0 = Instant::now();
        let range_batch_size = self.config.range_batch_size;
        let mut batch = Vec::with_capacity(range_batch_size);
    ...
    async fn send_flush_cmd(&mut self) -> anyhow::Result<()> {
        if self.engine.is_memtable_empty() {
            return Ok(());
        }
        ...
        if let Err(e) = self.io_cmd_sender.send(resync).await {
        ...
        if let Err(e) = self.io_cmd_sender.send(flush).await {
    ...
    #[instrument(skip_all, err)]
    async fn send_compaction_cmd(&mut self) -> anyhow::Result<()> {
        if self.engine.total_sstable_size() < self.engine.memtable_capacity() as u64 * 8 {
            return Ok(());
        }

        let count = (self.config.max_sstable_count / 2).max(2);
        ...
        if let Err(e) = self.io_cmd_sender.send(cmd).await {
