...
use crate::{
    proto,
    storage::{
        BloomFilter, IndexBlock, MemTable, SSTableMeta, Wal,
    ...
    pub fn open(base_dir: PathBuf, memtable_limit: usize) -> anyhow::Result<(Self, bool)> {
        if !base_dir.exists() {
            fs::create_dir_all(&base_dir)
                .context("engine.open: failed to create base directory")?;
        }
        fs::create_dir_all(base_dir.join(Wal::WAL_DIR))
            .context("engine.open: failed to create WAL directory")?;
