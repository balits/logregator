...
impl Server {
    ...
    pub async fn new(addr: Option<SocketAddr>) -> anyhow::Result<Self> {
        ...
        let socket = socket2::Socket::new(domain, socket2::Type::STREAM, Some(socket2::Protocol::TCP))
            .context("server.new: failed to create socket")?;
        socket.set_reuse_address(true)
            .context("server.new: failed to set SO_REUSEADDR")?;
        socket.set_nonblocking(true)
            .context("server.new: failed to set nonblocking")?;
        socket.bind(&addr.into())
            .context("server.new: failed to bind")?;
        socket.listen(1024)
            .context("server.new: failed to listen")?;
        let listener: std::net::TcpListener = socket.into();
        let listener = TcpListener::from_std(listener)
            .context("server.new: failed to create tokio TcpListener")?;
        Ok(Self { listener })
    }
    ...
    pub async fn run_main_loop(
        ...
        let server_addr = self
            .listener
            .local_addr()
            .context("server.main_loop: failed to get local addr")?;
    ...
    async fn handle_conn(
        ...
        while let Some(msg) = framed.next().await {
            ...
            let msg = msg.context("server.handle_conn: failed to read client message")?;
            ...
            match msg {
                proto::ClientMessage::Insert(i) => {
                    ...
                    let reply = res
                        ...
                        .unwrap_or_else(|e| ServerMessage::Error(format!("{:#}", e)));
                ...
                proto::ClientMessage::BatchInsert(b) => {
                    ...
                    let res = rx.await.context(
                        "server.handle_conn(cmd=BATCH_INSERT): failed to await cmd result",
                    )?;
                    let reply = res
                        .map(|_| ServerMessage::InsertOk)
                        .unwrap_or_else(|e| ServerMessage::Error(format!("{:#}", e)));
                    metrics.server.frames_written.inc(1);
                    framed.send(reply).await.context(
                        "server.handle_conn(cmd=BATCH_INSERT): failed to reply to connection",
                    )?;
                }
                proto::ClientMessage::Range(r) => {
                    ...
                    while let Some(records) = record_recv.recv().await {
                        record_frames += records.len() as u64;
                        for rec in &records {
                            write_record_frame(framed.write_buffer_mut(), rec);
                        }
                        if framed.write_buffer().len() > FLUSH_THRESHOLD {
                            framed.flush().await.context(
                                "server.handle_conn(cmd=RANGE): failed to flush records",
                            )?;
                        }
                    }
                    framed
                ...
                proto::ClientMessage::Ping => {
                    ...
                    framed
                        ...
                        .await
                        .context("server.handle_conn(cmd=PING): failed to reply")?;
                }
...
mod integration_tests {
    ...
    async fn test_insert_then_range() {
        ...
        client
            .insert(make_insert(1, 100, "sys", "cpu normal"))
            .await
            .unwrap();
    ...
    async fn test_range_invalid_ts() {
        ...
        assert!(
            err.to_string()
                .contains("end_ts cannot be smaller than start_ts")
        );
    }
    ...
    async fn test_multiple_inserts_range_subset() {
        ...
        for i in 0..10 {
            client
                .insert(make_insert(1, i * 10, "sys", &format!("val_{}", i)))
                .await
                .unwrap();
        }
